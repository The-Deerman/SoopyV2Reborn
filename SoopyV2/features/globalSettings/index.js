...
this.registerStep(false,60,()=>{return Promise.resolve().then(()=>{return(()=>{
if(!(this.ahAlerts.length===0)){return Promise.resolve().then(()=>{return(
fetch("https://moulberry.codes/auction.json").json())}).then((_resp)=>{let data=_resp;
if(!!data.success){
...
json())}).then((_resp)=>{let json=_resp;
...
soopyWeight(user){return Promise.resolve().then(()=>{

ChatLib.chat(this.FeatureManager.messagePrefix+"Finding senither weight for "+user);return(

fetch("https://soopy.dev/api/v2/player/"+user).json())}).then((_resp)=>{let userData=_resp;return(()=>{
if(!userData.success){
...
fetch("https://soopy.dev/api/v2/player_skyblock/"+userData.data.uuid).json())}).then((_resp)=>{let sbData=_resp;
...
ChatLib.chat("&c"+ChatLib.getChatBreak("-"))}})}})()}).then(()=>{})}
