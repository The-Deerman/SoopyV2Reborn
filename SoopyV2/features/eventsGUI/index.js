...
class EventsPage extends GuiPage {
    ...
    updateNotInEvent() {
        ...
        this.pages[0].addChild(new SoopyTextElement().setText("\xA70You are not currently in any events").setMaxTextScale(3).setLocation(0.2, 0.1, 0.6, 0.2));
        this.pages[0].addChild(new SoopyTextElement().setText("\xA70If you have a join code enter it here").setMaxTextScale(1).setLocation(0.3, 0.4, 0.4, 0.1));
        let joinBox = new TextBox().setPlaceholder("Code here").setLocation(0.3, 0.5, 0.4, 0.1);
        this.pages[0].addChild(joinBox);

        this.pages[0].addChild(new ButtonWithArrow().setLocation(0.35, 0.6, 0.3, 0.1).setText("\xA70Join Event").addEvent(new SoopyMouseClickEvent().setHandler(() => {
    ...
    updateInEvent(data) {
        ...
        this.pages[0].addChild(new SoopyTextElement().setText("\xA70You are curently in an event managed by \xA76" + data.admin).setMaxTextScale(3).setLocation(0.1, 0.05, 0.8, 0.2));

        if (!data.members[Player.getUUID().toString().replace(/-/g, "")]) {
            this.pages[0].addChild(new ButtonWithArrow().setText("\xA70Join").setLocation(0.2, 0.2, 0.6, 0.1).addEvent(new SoopyMouseClickEvent().setHandler(() => {
                this.pages[0].clearChildren();
                this.pages[0].addChild(new BoxWithLoading().setLocation(0.3, 0.3, 0.4, 0.4));
        ...
        Object.values(data.members).sort((a, b) => b.progress - a.progress).forEach((m, i) => {
            ...
            let nameLine = new SoopyTextElement().setText(`${isPlayer ? "\xA7d" : "\xA70"}#${i + 1} ${m.username}`).setLocation(0, i * 0.05, 0.5, 0.05).setLore(["Last updated " + timeSince(m.timestamp) + " ago", "Currently: " + (m.online ? "\xA7aOnline" : "\xA7cOffline")]);
            ...
            Object.keys(m.current.weights).map((w) => {
                ...
                lore.push(firstLetterWordCapital(l[0].replace(/_/g, " ").replace(/^.+? /, "")) + ": \xA7e" + numberWithCommas(Math.floor(l[1])));
            });
        ...
        if (playerPosition >= 0) {
            this.pages[0].addChild(new SoopyTextElement().setText("\xA70You are #" + playerPosition + " with +" + Math.floor(data.members[Player.getUUID().toString().replace(/-/g, "")].progress)).setMaxTextScale(2).setLocation(0.2, 0.2, 0.6, 0.1));
        }
        ...
        sideBarElm.addChild(new SoopyTextElement().setText("\xA70Event Settings").setMaxTextScale(3).setLocation(0.1, 0, 0.8, 0.2));

        if (Player.getUUID().toString().replace(/-/g, "") === data.admin) {
            sideBarElm.addChild(new ButtonWithArrow().setText("\xA70Change").setMaxTextScale(3).setLocation(0.1, 0.2, 0.8, 0.2).addEvent(new SoopyMouseClickEvent().setHandler(() => {
    ...
    regenGuiElements() {
        ...
        if (this.leaderboardElm) {
            ...
            this.leaderboardChildren.forEach((c) => {
                if (c.timestamp) {
                    c.setLore(["Last updated " + timeSince(c.timestamp) + " ago", "Currently: " + (c.online ? "\xA7aOnline" : "\xA7cOffline")]);
                }
            });
        }
    }
    ...
    pollEventData(admin) {
        if (!admin) {
            this.updateNotInEvent();
            this.pages[0].addChild(new SoopyTextElement().setText("\xA7cInvalid code").setMaxTextScale(3).setLocation(0.2, 0.7, 0.6, 0.2));
            return;
        }
        this.pages[0].clearChildren();
        this.pages[0].addChild(new SoopyTextElement().setText("\xA70Join \xA76" + admin + "\xA70's event?").setMaxTextScale(3).setLocation(0.2, 0.2, 0.6, 0.2));
        this.pages[0].addChild(new ButtonWithArrow().setText("\xA70Join").setLocation(0.4, 0.4, 0.4, 0.3).addEvent(new SoopyMouseClickEvent().setHandler(() => {
            socketConnection.joinEvent(this.code);
            this.pages[0].clearChildren();
            this.pages[0].addChild(new BoxWithLoading().setLocation(0.3, 0.3, 0.4, 0.4));
        })));
        this.pages[0].addChild(new ButtonWithArrow().setText("\xA70Cancel").setLocation(0.2, 0.4, 0.2, 0.3).setDirectionRight(false).addEvent(new SoopyMouseClickEvent().setHandler(() => {
            this.updateNotInEvent();
        })));
    }
    ...
    joinEventResult(data) {
        this.pages[0].clearChildren();
        if (data.success) {
            this.pages[0].addChild(new SoopyTextElement().setText("\xA70Joined event!").setMaxTextScale(3).setLocation(0.2, 0.2, 0.6, 0.2));
            this.pages[0].addChild(new ButtonWithArrow().setText("\xA70Ok").setLocation(0.3, 0.4, 0.4, 0.4).addEvent(new SoopyMouseClickEvent().setHandler(() => {
                this.pages[0].clearChildren();
                this.pages[0].addChild(new BoxWithLoading().setLocation(0.3, 0.3, 0.4, 0.4));
            ...
            this.pages[0].addChild(new SoopyTextElement().setText("\xA70Unable to join event!").setMaxTextScale(3).setLocation(0.2, 0.2, 0.6, 0.1));
            this.pages[0].addChild(new SoopyTextElement().setText("\xA70" + data.reason).setMaxTextScale(3).setLocation(0.2, 0.3, 0.6, 0.1));
            this.pages[0].addChild(new ButtonWithArrow().setText("\xA70Ok").setLocation(0.3, 0.4, 0.4, 0.4).addEvent(new SoopyMouseClickEvent().setHandler(() => {
                this.pages[0].clearChildren();
                this.pages[0].addChild(new BoxWithLoading().setLocation(0.3, 0.3, 0.4, 0.4));
