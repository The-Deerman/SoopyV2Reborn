...
class Events extends Feature {
    ...
    step_1fps() {
        ...
        this.Mobs = this.Mobs.filter((e) => !e.getEntity()["field_70128_L"]);
    }
    ...
    renderWorld(ticks) {
        ...
        if (this.showingWaypoints) {
            ...
            this.burrialData.locations.forEach((loc, i) => {
                ...
                if (!loc.clicked) {
                    ...
                    if (loc.fromApi) {
                        name = (loc.nearest ? "\xA7c" : "\xA7a") + "(" + (loc.chain + 1) + "/4) " + typeReplace[loc.type] + " burrial";
                    } else {
                        name = (loc.nearest ? "\xA7c" : "\xA7a") + typeReplace[loc.type] + " burrial";
                    }
        ...
        if (this.otherInquisWaypoints.getValue()) {
            Object.keys(this.slayerLocationDataH).forEach((key) => {
                drawCoolWaypoint(this.slayerLocationDataH[key][0][0] || 0, this.slayerLocationDataH[key][0][1] || 0, this.slayerLocationDataH[key][0][2] || 0, 255, 0, 0, { name: "\xA7c" + (key || "ERROR") + "'s inquis" });
            });
        }
    }
    ...
    spawnParticle(particle, type, event) {
                ...
                if (this.locs.length < 100 && this.locs.length === 0 || particle.getX() + particle.getY() + particle.getZ() !== this.locs[this.locs.length - 1][0] + this.locs[this.locs.length - 1][1] + this.locs[this.locs.length - 1][2]) {
                    ...
                    if (this.locs.length > 5 && this.guessPoint) {
                        ...
                        while (distCovered < this.distance2 && i < 10000) {
                            ...
                            let dist = distMultiplier * (0.06507 * i + 0.259);
                        ...
                        if (d1 < d2) {
                            this.guessPoint2 = [Math.floor(p1[0]), Math.floor(p1[1]), Math.floor(p1[2])];
                        } else {
                            this.guessPoint2 = [Math.floor(p2[0]), Math.floor(p2[1]), Math.floor(p2[2])];
                        }
                    }
                }
        ...
        if (this.showingWaypoints && this.loadFromParticles.getValue()) {
            ...
            if (particle.toString().startsWith("EntityEnchantmentTableParticleFX, ")) {
                ...
                if (particle.toString().startsWith("EntityCrit2FX, ")) {
                    foundCrit = true;

                    isMob = particle.getUnderlyingEntity().func_70534_d() > 0.5;
                } else
                    if (particle.toString().startsWith("EntityFootStepFX, ")) {
                        foundStep = true;
                    } else
