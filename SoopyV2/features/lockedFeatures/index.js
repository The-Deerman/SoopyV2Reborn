    ...
    step() {
        ...
        if (!this.eventCommand) {
            this.eventCommand = this.registerCommand("eventlb", () => {
                SoopyV2Server.lbdatathing.forEach((u, i) => {
                    ...
                    text += "\xA76#" + (i + 1);
                    text += "\xA77 - ";
                    text += "\xA7e" + u.username;
                    text += "&7: \xA7r" + numberWithCommas(Math.round(parseFloat(u.startingAmount)));
                    if (u.progress) text += " \xA77(" + (u.progress > 0 ? "+" : "-") + Math.abs(Math.round(u.progress)) + "/h)";
                    ChatLib.chat(text);
                });
            });
        }
        ...
        SoopyV2Server.lbdatathing.forEach((u, i) => {
            ...
            if (i === playerPos - 1 || i === playerPos || i === playerPos + 1 || playerPos === 0 && i === playerPos + 2) {
                ...
                text += "\xA77 - ";
                text += "\xA7e" + u.username;
                text += "&7: \xA7f" + numberWithCommas(Math.round(parseFloat(u.startingAmount)));
                if (u.progress) text += " \xA77(" + (u.progress > 0 ? "+" : "-") + Math.abs(Math.round(u.progress)) + "/h)";
                text += "\n";
            }
        });
