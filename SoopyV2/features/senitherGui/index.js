...
class SettingPage extends GuiPage {
    ...
    loadPlayerInformation() {
        return Promise.resolve().then(() => {
            return (() => {
                if (!this.updatingPlayerInfo) {
                    return Promise.resolve().then(() => {
                        this.updatingPlayerInfo = true;
                        this.playerInformationUpdated = true; return (
                            fetch("https://hypixel-app-api.senither.com/leaderboard/players?perPage=100&page=1&sort=" + this.playerSortThing + (this.playerSearch ? "&username=" + this.playerSearch : "")).json())
                    }).then((_resp) => {
                        let players = _resp;
    ...
    loadFirstPageInformation() {
        return Promise.resolve().then(() => {
            return (
                fetch("https://hypixel-app-api.senither.com/leaderboard/stats").json())
        }).then((_resp) => {
            let stats = _resp;
            ...
            this.playerButton.setDesc("\xA70Players tracked: " + numberWithCommas(stats.data.players)); return (
                ...
                fetch("https://hypixel-app-api.senither.com/leaderboard/history?perPage=10&page=1").json())
        }).then((_resp) => {
            let leaveAndJoins = _resp;
