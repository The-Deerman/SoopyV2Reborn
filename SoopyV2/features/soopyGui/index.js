...
class SoopyGui extends Feature {
    ...
    onEnable() {
        ...
        let title = new SoopyTextElement().setText("\xA70SoopyV2!").setMaxTextScale(3).setLocation(0.1, 0.05, 0.5, 0.1);
        this.categoryPage.addChild(title);

        let discordButton = new ButtonWithArrow().setText("\xA70Discord").setLocation(0.7, 0.05 + 0.0125, 0.25, 0.075);
    ...
    updateButtons() {
        ...
        if (this.getPages()) this.getPages().forEach((p, i) => {
            let settingsButton = new ButtonWithArrow().setText("\xA70" + p.name).setLocation(0, 0.175 * i, 1, 0.15);
    ...
    clickedOpen(category, anim = true) {
        ...
        if (Date.now() - this.lastClickedOpen < 100) return;
