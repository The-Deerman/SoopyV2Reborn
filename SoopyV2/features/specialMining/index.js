...
class PowderAndScatha extends Feature {
    ...
    onEnable() {
        ...
        this.registerStep(true, 5, () => {
            ...
            if (!this.holdingJunglePick) return
            let inGui = Client.isInGui(), globalSludge = this.useGlobalSludgeJuiceDrop.getValue()
            let j = 0, sludge = 0;
            if (!inGui && globalSludge) {
                [...Player.getInventory().getItems()].forEach(i => {
                    j++;
                    if (j == 9 || j > 36 || !i) return //only do the 1-8 hot bar slots & inv slots
                    let itemName = i.getName()
                    let itemNameReformat = itemName.removeFormatting()
                    if (itemNameReformat?.startsWith("Sludge Juice")) {
                        sludge += itemNameReformat?.getNBT().getDouble("Count")
                    }
                })
            }
            if (sludge > this.sludgeCounter) this.addTreasure("Jungle", "Sludge Juice", this.sludgeCounter - sludge)
            this.sludgeCounter = sludge
        })
