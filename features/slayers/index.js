...
if(eArray[1]==="wolf"){
return name.endsWith(" \xA7e0\xA7f/\xA7a15000\xA7c\u2764")&&name.endsWith(" \xA7e0\xA7f/\xA7a31150\xA7c\u2764");
}
let areaMiniHPSuffix={
zombie:" \xA7e0\xA7f/\xA7a45000\xA7c\u2764",
enderman:" \xA7e0\xA7f/\xA7a8M\xA7c\u2764",
blaze:" \xA7e0\xA7f/\xA7a30M\xA7c\u2764"};
...
if(this.BoxAroundMiniboss.getValue()&&!this.bossSpawnedMessage&&this.Miniboss[this.lastSlayerType]?.has(MobName)&&!this.minibossEntity.map((a)=>a[0].getUUID().toString()).includes(name.getUUID().toString())&&!Name.endsWith(" \xA7e0\xA7c\u2764")){
...
name.getEntity()["func_174805_g"](false);
}
}
}
});
...
if(this.cannotFindEmanBoss&&(e["field_70165_t"]-Player.getX())**2+(e["field_70163_u"]-Player.getY())**2+(e["field_70161_v"]-Player.getZ())**2<5){
this.emanBoss=new Entity(e);
this.assignActualEmanBoss(this.emanBoss);
this.cannotFindEmanBoss=false;
}
...
if(e instanceof net.minecraft.entity.item.EntityArmorStand&&e["func_95999_t"]()&&this.blazeTowerDink.getValue()){
let name=ChatLib.removeFormatting(e["func_95999_t"]());
let isPiller=true;
...
if(this.emanBoss&&this.emanBoss.getEntity()["field_70128_L"]){
this.emanBoss=undefined;
this.actualEmanBoss=undefined;
}
...
eArray.getEntity()["func_174805_g"](false);
}
...
this.summonEntity=this.summonEntity?.filter((e)=>!e.getEntity()["field_70128_L"]);
if(this.summonHPGuiElement.getValue()){
this.summonHPElement.setText(summonHpFloatText);
}
this.minibossEntity.forEach((eArray)=>{
if(eArray[0].getEntity()["field_70128_L"]||eArray[0].getName().endsWith(" \xA7e0\xA7c\u2764")){
this.minibossEntity.splice(this.minibossEntity.indexOf(eArray));
}
});
this.areaMiniEntity.forEach((eArray)=>{
if(eArray[0].getEntity()["field_70128_L"]||this.areaMiniIsDead(eArray)){
this.areaMiniEntity.splice(this.areaMiniEntity.indexOf(eArray));
}
});
this.arachneKeeperEntity=this.arachneKeeperEntity.filter((e)=>!e.getEntity()["field_70128_L"]);
this.eyeE=this.eyeE.filter((e)=>!e.getEntity()["field_70128_L"]);
this.candidateBoss=this.candidateBoss.filter((e)=>!e["field_70128_L"]);
...
this.beaconPoints[e["func_110124_au"]().toString()].push([e["field_70165_t"]+0.5,e["field_70163_u"]+0.7,e["field_70161_v"]+0.5]);
});
...
if(name.split(" ")[2]==="\xA7e0\xA7c\u2764")return;
tempEntity.push(x[0]);
tempArray.push(name);
});
...
if(this.pillerE){
if(this.pillerE.getEntity()["field_70128_L"])this.pillerE=undefined;
}
...
if(this.emanLazerTimer.getValue()&&this.actualEmanBoss&&this.actualEmanBoss["func_70115_ae"]()){
