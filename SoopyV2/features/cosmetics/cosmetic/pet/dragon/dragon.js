...
class DragonPet extends Pet {
    ...
    onCommand(option, ...args) {
        ...
        if (option === "come") {
            ...
            ChatLib.chat("Dragon pet coming to current location!");
            return;
        }
        if (option === "tp") {
            this.x = Player.getX();
            this.y = Player.getY();
            this.z = Player.getZ();
            ChatLib.chat("Dragon pet tping to current location!");
            return;
        }
        if (option === "goto") {
            let [x, y, z] = args;
            ...
            ChatLib.chat("Dragon pet going to " + this.travelToPosition.join(", ") + "!");
            return;
        }
        if (option === "follow") {
