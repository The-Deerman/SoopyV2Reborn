...
class NetworthPage extends GuiPage {
    ...
    updateData(player, profIn) {
        var _test; return Promise.resolve().then(() => {
            ...
            this.statArea.addChild(this.loadingElm); return (
                ...
                fetch("https://soopy.dev/api/v2/player/" + player).json())
        }).then((_resp) => {
            let playerData = _resp; return (() => {

                if (!(player !== this.playerLoad)) {
                    return (() => {
                        return (() => {
                            ...
                            if (!playerData.success) {
                                ...
                                return Promise.resolve().then(() => {
                                    return (
                                        ...
                                        fetch("https://soopy.dev/api/v2/player_skyblock/" + playerData.data.uuid + "?networth=true").json())
                                }).then((_resp) => {
                                    let skyblockData = _resp; return (() => {
                                                    ...
                                                    if (!skyblockData.success) {
                                                        ...
                                                        return Promise.resolve().then(() => {
                                                            ...
                                                            Object.keys(nwData.types).sort((a, b) => nwData.types[b].total - nwData.types[a].total).forEach((name, i) => {
                                                                ...
                                                                selectedProf === skyblockData.data.stats.bestProfileId; return (() => {
                                                                    if (_test) {
                                                                        return Promise.resolve().then(() => {
                                                                            return (
                                                                                fetch("https://soopy.dev/api/v2/leaderboard/networth/user/" + playerData.data.uuid).json())
                                                                        }).then((_resp) => { let leaderboardData = _resp })
                                                                    }
                                                                })()
                                                        }).then(() => {
                                                            if (!(_test && player !== this.playerLoad)) {

                                                                if (_test && leaderboardData.success) { nameElm.setText("\xA70#" + numberWithCommas(leaderboardData.data.data.position + 1) + " " + playerData.data.stats.nameWithPrefix.replace(/§f/g, "\xA77")) }
                                                            }
                                                        })
                                                    }
                                                })()
    ...
    sidebarSearch(user) {
        return Promise.resolve().then(() => {
            return (
                fetch("https://soopy.dev/api/v2/leaderboard/networth/user/" + user).json())
        }).then((_resp) => {
            let data = _resp;
            if (!!data.success) {
    ...
    goToLeaderboardPage(page, scroll = true) {
        return Promise.resolve().then(() => {
            ...
            if (scroll) { this.leaderboardArea.location.scroll.y.set(0, 100) } return (
                ...
                fetch("https://soopy.dev/api/v2/leaderboard/networth/" + page).json())
        }).then((_resp) => {
            let data = _resp;
