...
class DungeonReadyGui {
    constructor() {
        ...
        this.soopyGui.element.addEvent(new SoopyKeyPressEvent().setHandler((...args) => this.keyPress(...args)));

        this.startButton = new ButtonWithArrow().
            setText("\xA70Start Dungeon").
            setLocation(0.25, 0.25, 0.5, 0.5).
            addEvent(new SoopyMouseClickEvent().setHandler(() => this.startDungeon()));
    ...
    keyPress(key, keyId) {
        if (keyId === 1) Client.currentGui.close();
        if (keyId === 18) Client.currentGui.close();
    }
}
