...
class Bestiary extends Feature {
    ...
    updateMobs() {
        ...
        for (let entity of entities) {
            ...
            let color = [0, 100, 100];
            if (name.includes("Butterfly")) {continue;
    ...
    apiLoad(data, dataType, isSoopyServer, isLatest) {
        ...
        Object.entries(currentProfile.bestiary).forEach(([key, val]) => {
            if (key.startsWith("kills_family_")) {
                ...
                let { level, killsLeft, killsForNext, isBald } = getBestiaryTier(family, val);
                ...
                if (isBald) delete this.bestiaryData[family];
            }
        });
        let changed = false;
