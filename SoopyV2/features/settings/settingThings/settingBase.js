import Promise from "../../../../PromiseV2"; import SoopyContentChangeEvent from "../../../../guimanager/EventListener/SoopyContentChangeEvent";
...
class SettingBase {
    constructor(name, description, defaultVal, settingId, module) {
        ...
        this.guiObject = new BoxWithTextAndDescription().setDesc("\xA70" + this.description.replace(/\n/g, "\n\xA70")).setText("\xA70" + this.name).setLocation(0, 0, 1, 0.15);

        this.settingObject = new SoopyGuiElement().setLocation(0.8, 0, 0.15, 1);

        this.guiObject.addChild(this.settingObject);

        this.helpButton = new BoxWithText().setText("\xA70?").setLocation(3, 3, 0.05, 0.5);
        ...
        this.helpButton.addEvent(new SoopyMouseClickEvent().setHandler(() => {
            return Promise.resolve().then(() => {
                module.FeatureManager.features.soopyGui.class.openSidebarPage(new SoopyGuiElement().setLocation(0.05, 0.05, 0.9, 0.9).setScrollable(true).addChild(new SoopyMarkdownElement().setLocation(0, 0, 1, 1).setText("Loading..."))); return (
    ...
    getHelp() {
        return Promise.resolve().then(() => {
            return helpDataLoader.getData(this.moduleId, this.settingId)
        })
    }
