...
class StatGraphPage extends GuiPage {
    constructor() {
        ...
        this.pages[0].addChild(new SoopyTextElement().setText("\xA70Stat Graphs").setMaxTextScale(3).setLocation(0.1, 0.05, 0.6, 0.1));
                    ...
                    return (() => {
                        return (() => {
                            ...
                            if (!playerData.success) {
                                ...
                                return Promise.resolve().then(() => {
                                    this.statArea.clearChildren();
                                    let nameElm = new SoopyTextElement().setText(playerData.data.stats.nameWithPrefix.replace(/§f/g, "\xA77")).setMaxTextScale(2).setLocation(0.1, 0.05, 0.5, 0.1);
                                    this.statArea.addChild(nameElm);
                                    this.statArea.addChild(this.loadingElm); return (

                                        fetch("https://soopy.dev/api/v2/player_skyblock/" + playerData.data.uuid).json())
                                }).then((_resp) => {
                                    let skyblockData = _resp;
                                    if (!(player !== this.playerLoad)) {
                                            ...
                                            fetch("https://soopy.dev/statgraphgenerations/" + playerData.data.uuid + "/" + selectedProf).json().then((graphData) => {
                                                ...
                                                new Thread(() => {
                                                    ...
                                                    Object.keys(graphData).forEach((type) => {
                                                        let typeElm = new SoopyTextElement().setText("\xA70" + firstLetterWordCapital(type.replace("total", ""))).setMaxTextScale(2).setLocation(0.1, y, 0.8, 0.1);
                                                        this.statArea.addChild(typeElm);
                                                        y += 0.15;
                                                        let graph = Object.keys(graphData[type])[0];
                                                        renderLibs.getImage(graphData[type][graph], true);
